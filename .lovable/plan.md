...
| 10 | **Macro Controller — `pro_0` plan credit balance retrieval** — when `WorkspacePlan === PRO_ZERO`, GET `/workspaces/{WorkspaceId}/credit-balance` and map `total_granted`/`total_remaining`/`total_billing_period_used` into `MacroCreditSummary`. IndexedDB cache (TTL ≥10 min, configurable via `Settings.ProZeroCreditBalanceCacheTtlMinutes`) + async SQLite `Workspaces` upsert. Right-click on a `pro_0` workspace copies `{ Workspace, CreditBalance }` JSON. Hard-fail with typed error on HTTP/network/parse failure (no silent fallback). All Enums/types/constants in their own files; function ≤8 lines, file ≤100 lines, no `any`/`unknown`/casts. **Awaiting `next` to implement.** | High | `spec/22-app-issues/110-macro-controller-pro-zero-credit-balance.md` + `mem://features/macro-controller/pro-zero-credit-balance` |
