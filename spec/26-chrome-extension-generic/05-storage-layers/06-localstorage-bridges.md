...
Document the localStorage TTL bridge pattern for moving short-lived data (e.g., bearer tokens) into the MAIN world, including the risks (cross-origin exposure, persistence) and required mitigations (TTL, scoped key prefix, cleanup hook).

---

## Source material to distill (private — do NOT cite in final body)

mem://auth/token-readiness-gate, mem://auth/token-retrieval-strategy
