...
Describe the two-stage builtin-script-guard pattern (detect missing/corrupted scripts → reseed from packaged copy), the hash-based reseed trigger (only reseed when source hash changes), and the storage-schema-version migrator.

---

## Source material to distill (private — do NOT cite in final body)

mem://architecture/self-healing-script-storage, mem://architecture/instruction-driven-seeding
